...
function BolgelerTab({ onOk, onErr }: { onOk: (s: string) => void; onErr: (s: string) => void }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="card p-6">
        <h3 className="font-bold text-charcoal-900">Bölge Ekle</h3>
...
function SifreTab({ onOk, onErr }: { onOk: (s: string) => void; onErr: (s: string) => void }) {
  ...
  async function save() {
    ...
    if (next1.length < 8) return onErr('Yeni şifre en az 8 karakter olmalı');
    const token = providerSession.get();
    if (!token) return;
    setLoading(true);
    try {
  ...
  return (
    <div className="card space-y-4 p-6">
      ...
      <Button disabled={loading || !current || !next1 || next1 !== next2} onClick={save}>
        Şifreyi Güncelle
      </Button>
    </div>
  );
