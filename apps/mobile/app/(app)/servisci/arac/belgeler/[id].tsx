...
import { Button, ErrorBanner, InfoBanner } from '../../../../../src/components/ui';
...
export default function VehicleBelgelerScreen() {
  ...
  const load = useCallback(async () => {
    if (!token || !id) return;
    try {
      const ds = await api.get<DocRow[]>(`/me/vehicles/${id}/documents`, token);
      setDocs(ds);
      setError(null);
    } catch (e) {
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Araç Belgeleri</Text>
        <View style={{ width: 32 }} />
      </View>
...
function DocumentCard({ row, onUpload }: { row: DocRow; onUpload: () => void }) {
  ...
  return (
    <View style={[styles.docCard, isApproved && styles.docCardApproved, isRejected && styles.docCardRejected, isMissing && styles.docCardMissing]}>
